...
public enum PicassoBigCache {
    ...
    private void init(Context context) {
        if (context == null) {
            throw new IllegalStateException("Must provide context to init PicassoBigCache!"); // fail
            // fast
        }
        context = context.getApplicationContext(); // need application context -
        // activity's context could
        // cause harm
        Picasso.Builder builder = new Picasso.Builder(context);
        builder.downloader(createBigCacheDownloader(context));
